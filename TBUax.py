...
#   SOL'N : Copying the method done for Gymnax Cartpole 
...
class TBU_Jax:
    ...
    def __init__(self):
        ...
        low=jnp.array([self.x_bounds[0], self.y_bounds[0], -2*jnp.pi, -2*jnp.pi, 0], dtype=jnp.float32)
        high=jnp.array([self.x_bounds[1], self.y_bounds[1], 2*jnp.pi, 2*jnp.pi, self.max_steps_per_episode], dtype=jnp.float32)
        self.action_space = spaces.Box(-1, 1, shape=(1,1), dtype=jnp.float32)
        self.observation_space = spaces.Box(low, high, shape=(1,5), dtype=jnp.float32)

    @partial(jit, static_argnums=(0,))
    def step(self, env_state, action):
        state, key = env_state
        x, y, theta_t, theta_c, step_counter = state # Auto-unpacking the array
        ...
        truncated = step_counter > self.max_steps_per_episode
        at_goal = jnp.logical_and(jnp.sqrt(x**2 + y**2) <= 3.0, jnp.abs(theta_t) <= 0.1)
        done = at_goal | jack_knifed | not_valid_loc | not_valid_angle | truncated
        # Goal-Contact Encoding with 10 for reaching the goal
        reward = (0 + at_goal)*10
        # Updating and returning the state
        env_state = jnp.array([x, y, theta_t, theta_c, step_counter]), key
        env_state = self._maybe_reset(env_state, done)
        # Making the observation (just does not include the time)
        observation = jnp.array([env_state[0][0], env_state[0][1], env_state[0][2], env_state[0][3]]) # JIT issues?
        # Returning everything
        return observation, env_state, reward, done, {}
    ...
    def _reset(self, key):
      ...
      new_state = jnp.array([160, new_y, new_theta_t, 0.0, 0.0], dtype=jnp.float32)
...
if __name__ == '__main__':
    ...
    while not done:
        action = 1
        obs, env_state, reward, done, info = env.step(env_state, 1)
